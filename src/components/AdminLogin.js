...
import adminLoginImage from "../Images/adminLogin.avif";
...
function AdminLogin(props) {
  ...
  const hanleSubmit = async (e) => {
    e.preventDefault();
    const response = await fetch(`${host}/api/adminAuth/login`, {
      method: "POST",
      headers: {
    ...
    <div className="flex items-center justify-center h-screen">
      <div className="loginContainer flex mx-5 my-10 justify-center items-center w-5/6 h-full rounded-3xl">
        <div
          ...
          style={{
            background: `url(${adminLoginImage})`,
            backgroundSize: "100% 100%",
            backgroundRepeat: "no-repeat",
          }}
        ></div>

        <div className="right h-[90%] flex w-1/2 items-center justify-center mr-5 rounded-r-lg border-black border-t border-b border-r">
